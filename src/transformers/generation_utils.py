...
class GenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        if "encoder_outputs" not in model_kwargs:
            ...
            # print("Fact & Opinion prediction")
            # print(model_kwargs["encoder_outputs"]["fact_preds_stacked"],
            #       model_kwargs["encoder_outputs"]["experience_preds_stacked"], "\n")
        return model_kwargs
    ...
    def _expand_inputs_for_generation(
        ...
        if is_encoder_decoder:
            ...
            encoder_outputs["attention_mask"] = encoder_outputs.attention_mask.index_select(
                0, expanded_return_idx.to(encoder_outputs.attention_mask.device)
            )
    ...
    @torch.no_grad()
    def predict_strategy(self, model_kwargs, device, bos_token_id, token_dict, batch_size):
        # ACT2INPUT = {0: 8030, 1: 8029}
        # REACT2INPUT = {0: 8031, 1: 8032, 2: 8033}
        # ADD2INPT = {0: 8034, 1: 8035}
        # print("Action, reaction, additional")
        # print(model_kwargs["encoder_outputs"]["action_prediction"], model_kwargs["encoder_outputs"]["reaction_prediction"], model_kwargs["encoder_outputs"]["additional_prediction"],"\n")
        # action = torch.argmax(model_kwargs["encoder_outputs"]["action_prediction"], -1).detach().tolist()
        # action = torch.tensor([ACT2INPUT[i] for i in action], dtype=torch.long).to(device)
        #
        # reaction = torch.argmax(model_kwargs["encoder_outputs"]["reaction_prediction"], -1).detach().tolist()
        # reaction = torch.tensor([REACT2INPUT[i] for i in reaction], dtype=torch.long).to(device)
        #
        # additional = torch.argmax(model_kwargs["encoder_outputs"]["additional_prediction"], -1).detach().tolist()
        # additional = torch.tensor([ADD2INPT[i] for i in additional], dtype=torch.long).to(device)

        strategy = model_kwargs["encoder_outputs"].get("strategy_prediction", None)
        if strategy is not None:
            strategy = torch.round(torch.sigmoid(strategy)).detach()
            # print(strategy)
            share_experience = torch.tensor(
                [token_dict["pad_token_id"] if i == 0 else token_dict["share_experience"] for i
                 in strategy[:, 0].tolist()],
                dtype=torch.long)
            share_knowledge = torch.tensor(
                [token_dict["pad_token_id"] if i == 0 else token_dict["share_knowledge"] for i in
                 strategy[:, 1].tolist()],
                dtype=torch.long)
            seek_experience = torch.tensor(
                [token_dict["pad_token_id"] if i == 0 else token_dict["seek_experience"] for i in
                 strategy[:, 2].tolist()],
                dtype=torch.long)
            seek_knowledge = torch.tensor(
                [token_dict["pad_token_id"] if i == 0 else token_dict["seek_knowledge"] for i in
                 strategy[:, 3].tolist()],
                dtype=torch.long)

        personality = model_kwargs["encoder_outputs"].get("personality_prediction", None)
        if personality is not None:
            # personality = torch.round(torch.sigmoid(personality)).detach()

            # agreeableness = torch.tensor(
            #     [token_dict["pad_token_id"] if i == 0 else token_dict["agreeableness"] for i in personality[:, 0].tolist()],
            #     dtype=torch.long)
            # openness = torch.tensor(
            #     [token_dict["pad_token_id"] if i == 0 else token_dict["openness"] for i in personality[:, 1].tolist()],
            #     dtype=torch.long)
            # conscientiousness = torch.tensor(
            #     [token_dict["pad_token_id"] if i == 0 else token_dict["conscientiousness"] for i in personality[:, 2].tolist()],
            #     dtype=torch.long)
            # extraversion = torch.tensor(
            #     [token_dict["pad_token_id"] if i == 0 else token_dict["extraversion"] for i in personality[:, 3].tolist()],
            #     dtype=torch.long)
            # neuroticism = torch.tensor(
            #     [token_dict["pad_token_id"] if i == 0 else token_dict["neuroticism"] for i in personality[:, 4].tolist()],
            #     dtype=torch.long)
            personality = torch.argmax(personality, 1).detach()
            # print(personality)
            agreeableness = torch.tensor([token_dict["pad_token_id"] if i == 0 else token_dict[
                "high_agreeableness"] if i == 1 else token_dict["low_agreeableness"] for i in
                                          personality[:, 0].tolist()], dtype=torch.long)
            openness = torch.tensor([token_dict["pad_token_id"] if i == 0 else token_dict[
                "high_openness"] if i == 1 else token_dict["low_openness"] for i in
                                          personality[:, 1].tolist()], dtype=torch.long)
            conscientiousness = torch.tensor([token_dict["pad_token_id"] if i == 0 else token_dict[
                "high_conscientiousness"] if i == 1 else token_dict["low_conscientiousness"] for i in
                                          personality[:, 2].tolist()], dtype=torch.long)
            extraversion = torch.tensor([token_dict["pad_token_id"] if i == 0 else token_dict[
                "high_extraversion"] if i == 1 else token_dict["low_extraversion"] for i in
                                          personality[:, 3].tolist()], dtype=torch.long)
            neuroticism = torch.tensor([token_dict["pad_token_id"] if i == 0 else token_dict[
                "high_neuroticism"] if i == 1 else token_dict["low_neuroticism"] for i in
                                          personality[:, 4].tolist()], dtype=torch.long)

        sos = torch.tensor([bos_token_id] * batch_size, dtype=torch.long)
        if personality is not None and strategy is not None:
            return torch.stack([agreeableness, openness, conscientiousness, extraversion, neuroticism,
                                share_experience, share_knowledge, seek_experience, seek_knowledge,
                                sos], -1).contiguous().to(device)
        elif personality is not None:
            return torch.stack([agreeableness, openness, conscientiousness, extraversion,
                                neuroticism, sos], -1).contiguous().to(device)
        elif strategy is not None:
            return torch.stack([share_experience, share_knowledge, seek_experience,
                                seek_knowledge, sos], -1).contiguous().to(device)
        else:
            return sos.unsqueeze(1).contiguous().to(device)
        # share_experience = torch.tensor([8031 if i == 0 else 8035 for i in strategy[:, 0].tolist()],
        #                                 dtype=torch.long).to(device)
        # share_knowledge = torch.tensor([8032 if i == 0 else 8036 for i in strategy[:, 1].tolist()],
        #                                dtype=torch.long).to(device)
        # seek_experience = torch.tensor([8029 if i == 0 else 8033 for i in strategy[:, 2].tolist()],
        #                                dtype=torch.long).to(device)
        # seek_knowledge = torch.tensor([8030 if i == 0 else 8034 for i in strategy[:, 3].tolist()],
        #                               dtype=torch.long).to(device)

        # sos = torch.tensor([bos_token_id] * strategy.shape[0], dtype=torch.long).to(device)
        # return torch.stack([share_experience, share_knowledge, seek_experience, seek_knowledge, sos], -1).to(device)
    ...
    def generate(
            ...
            strategy_prediction: Optional[bool] = False,
        ...
        if self.config.is_encoder_decoder:
            ...
            if strategy_prediction:
                predicted_strategy = self.predict_strategy(model_kwargs, encoder_input_ids.device, bos_token_id,
                                                           token_dict, input_ids.shape[0])
            else:
                predicted_strategy = torch.tensor([bos_token_id] * encoder_input_ids.shape[0],
                                                  dtype=torch.long).unsqueeze(-1).to(encoder_input_ids.device)
            if "decoder_input_ids" in model_kwargs and model_kwargs["decoder_input_ids"] is not None:
                # if self.config.trait_in_decoder:
                #     input_ids = model_kwargs.pop("decoder_input_ids")
                #     input_ids = torch.cat([input_ids, predicted_strategy], -1)
                # else:
                #     input_ids = model_kwargs.pop("decoder_input_ids")
                input_ids = model_kwargs.pop("decoder_input_ids")
            else:
                input_ids = predicted_strategy

            # """ Do sample only if NOT sharing knowledge. """
            # if any([8034 in i or 8036 in i for i in input_ids.tolist()]):
            #     do_sample = False
    ...
    def sample(
        ...
        # if model is an encoder-decoder, retrieve encoder attention weights and hidden states
        # print("model_kwargs",model_kwargs["encoder_outputs"]["last_hidden_state"].shape, model_kwargs["encoder_outputs"]["attention_mask"].shape)
        if return_dict_in_generate and self.config.is_encoder_decoder:
            encoder_attentions = model_kwargs["encoder_outputs"].get("attentions") if output_attentions else None
            encoder_hidden_states = (
                model_kwargs["encoder_outputs"].get("hidden_states") if output_hidden_states else None
