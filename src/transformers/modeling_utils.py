...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        for k, v in state_dict.items():
            if ("encoder" in k or "decoder" in k) and "model." not in k:
                tmp["model." + k] = v
            else:
                tmp[k] = v
        state_dict = copy.deepcopy(tmp)
        missing_keys = []
        unexpected_keys = []
        error_msgs = []
        ...
        if from_tf:
            ...
            for k, v in state_dict_cp.items():
                # print(k, v.shape)
                if "encoder.layer" in k:
                    state_dict[k.replace("encoder", "encoder.fact_encoder")] = v
                    state_dict[k.replace("encoder", "encoder.strategy_encoder")] = v
                    state_dict[k.replace("encoder", "encoder.ctx_encoder")] = v

                if "embed_positions" in k:
                    try:
                        print("Performed surgery on ", k)
                        if "encoder" in k:
                            new_wt = model.get_parameter(k.replace("encoder", "encoder.ctx_encoder")).data
                            pre_wt = v.data
                            new_wt[:pre_wt.shape[0], :] = pre_wt
                            state_dict[k.replace("encoder", "encoder.fact_encoder")] = new_wt
                            state_dict[k.replace("encoder", "encoder.strategy_encoder")] = new_wt
                            state_dict[k.replace("encoder", "encoder.ctx_encoder")] = new_wt
                            # state_dict[k.replace("encoder", "encoder.ctx_strategy_encoder")] = new_wt
                        else:
                            new_wt = model.get_parameter(k).data
                            pre_wt = v.data
                            new_wt[:pre_wt.shape[0], :] = pre_wt
                            state_dict[k].data = new_wt
                    except Exception as e:
                        print("Exception", e)
                if "embed_tokens" in k and "encoder" in k:
                    state_dict[k.replace("encoder", "encoder.fact_encoder")] = v
                    state_dict[k.replace("encoder", "encoder.strategy_encoder")] = v
                    state_dict[k.replace("encoder", "encoder.ctx_encoder")] = v
                    # state_dict[k.replace("encoder", "encoder.ctx_strategy_encoder")] = v
