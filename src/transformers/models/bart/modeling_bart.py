...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None, token_type_embeddings=None):
...
class BartCustomEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, shared_emb, shared_token_type_embeddings):
        super().__init__(config)

        padding_idx, vocab_size = config.pad_token_id, config.vocab_size
        self.ctx_encoder = BartEncoder(config, shared_emb, shared_token_type_embeddings)
        ...
        if self.predict_personality:
            # self.personality_pred1 = nn.Sequential(nn.Linear(config.d_model, 512), nn.GELU())
        ...
        if config.ablation_remove != "fact":
            self.fact_encoder = BartEncoder(config, shared_emb, shared_token_type_embeddings)
            self.fact_encoder_attn = BartAttention(
                config.d_model,
                config.decoder_attention_heads,
                dropout=config.attention_dropout,
                is_decoder=False,
        ...
        if config.ablation_remove != "all":
            self.strategy_encoder = BartEncoder(config, shared_emb, shared_token_type_embeddings)
            self.strategy_fc = nn.Linear(config.d_model*2, config.d_model)
            self.strategy_encoder_attn = BartAttention(
                config.d_model,
                config.decoder_attention_heads,
                dropout=config.attention_dropout,
                is_decoder=False,
    ...
    def forward(
        ...
        if fact_input_ids is not None:
            ...
            for ix, fact in enumerate(fact_input_ids.permute(1, 0, 2)):  # 5, batch, seq_len
                ...
                fact_enc_op = self.fact_encoder(input_ids=fact,
                                                attention_mask=f_attention_mask)  # batch, seq_len, h_dim
            ...
            if fact_label is not None:
                fact_label = torch.zeros_like(fact_label).scatter_(-1, torch.argmax(fact_label, -1).unsqueeze(-1),
                                                                   1) * fact_label # Added *fact_label
                h_fact = (fact_encs_stacked * fact_label.unsqueeze(-1).unsqueeze(-1)).sum(
                    1)  # batch, fact_len, hid dim
            else:
        ...
        if fact_input_ids is not None:
            encoder_hidden_states = torch.cat([encoder_outputs[0], h_fact],
                                              1).contiguous()  # batch, (ctx_len + fact_len), hid_dim
            attention_mask = torch.cat([attention_mask, h_fact_attn_mask],
                                       -1).contiguous()  # batch, (ctx_len + fact_len)
        else:
...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        ...
        self.encoder = BartCustomEncoder(config, self.shared, self.token_type_embeddings)
        self.decoder = BartDecoder(config, self.shared, self.token_type_embeddings)
        self.padding_idx = padding_idx
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        return super(BartModel, cls).from_pretrained(pretrained_model_name_or_path, *model_args, **kwargs)
    ...
    def forward(
        ...
        return Seq2SeqModelOutputCustom(
            ...
            fact_preds=encoder_outputs["fact_preds_stacked"],
...
class AdaptiveSmoothingLoss(nn.Module):
    ...
    def forward(self, pred, target, strategy_labels):
        ...
        return torch.sum(torch.sum(-new_dist * pred, -1)) / mask.sum()
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        return super(BartForConditionalGeneration, cls).from_pretrained(
    ...
    def forward(
        ...
        if fact_labels is not None:
            loss_fact_select = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(2.3).to(fact_labels.device))
            fact_selection_loss = loss_fact_select(outputs.fact_preds.view(-1), fact_labels.view(-1))

        """ Calculate Strategy Selection Loss """
        if strategy_labels is not None:
            loss_strategy_select = nn.BCEWithLogitsLoss(
                pos_weight=torch.tensor([1.0, 1.0, 2.0, 3.0]).to(strategy_labels.device))
            strategy_pred_loss = loss_strategy_select(outputs.strategy_prediction, strategy_labels)

        if personality_labels is not None:
            loss_personality_select = nn.CrossEntropyLoss()#nn.BCEWithLogitsLoss()
