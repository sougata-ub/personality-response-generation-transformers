...
class BlenderbotModel(BlenderbotPreTrainedModel):
    def __init__(self, config: BlenderbotConfig):
        ...
        self.fact_encoder = BlenderbotEncoder(config, self.shared)
        self.decoder = BlenderbotDecoder(config, self.shared)

        self.l_attn = nn.Linear(config.d_model * 2, config.d_model)
        self.v = nn.Parameter(torch.randn(config.d_model))
        self.init_weights()
    ...
    def forward(
        ...
        distractor_input_ids=None,
        distractor_attention_mask=None,
        head_mask=None,
        ...
        if encoder_outputs is None:
            ...
            )  # batch, seq len, hid dim
            fact_outputs, fact_attn_op = [], []
            for ix, fact in enumerate(fact_input_ids):
                fact_enc_op = self.fact_encoder(input_ids=fact, attention_mask=fact_attention_mask[ix],
                                                head_mask=None, inputs_embeds=None, output_attentions=output_attentions,
                                                output_hidden_states=output_hidden_states, return_dict=return_dict)
                fact_outputs.append(fact_enc_op["last_hidden_state"])

            fact_outputs_stacked = torch.stack(fact_outputs).permute(1, 0, 2, 3)  # batch, n_facts, fact_len, hid dim
            fact_attn_op = torch.stack([i.sum(0) for i in fact_outputs])  # n_facts, batch, hid dim
            repeat_enc_hidden = encoder_outputs["last_hidden_state"].sum(1).repeat(fact_attn_op.size(0), 1, 1)
            concatenate_hidden = torch.cat([repeat_enc_hidden, fact_attn_op], -1)  # n_facts, batch, hidden_size*2
            energy = self.l_attn(concatenate_hidden).tanh()  # n_facts, batch, hidden_size
            attn_energies = torch.sum(self.v * energy, dim=-1).permute(1, 0)  # batch, n_facts
            attn_energies = torch.softmax(attn_energies, -1).unsqueeze(-1).unsqueeze(-1)  # batch, n_facts, 1, 1
            fact_output = (attn_energies * fact_outputs_stacked).sum(1)  # batch, fact_len, hid dim

            encoder_outputs = (torch.cat([encoder_outputs["last_hidden_state"], fact_output], 1).contiguous())
        ...
        decoder_distractor_outputs = self.decoder(
        ...
        if not return_dict:
            ...
            return_list = [Seq2SeqModelOutput(last_hidden_state=decoder_outputs.last_hidden_state,
                           ...
                           Seq2SeqModelOutput(last_hidden_state=decoder_distractor_outputs.last_hidden_state,
                                              past_key_values=decoder_distractor_outputs.past_key_values,
                                              decoder_hidden_states=decoder_distractor_outputs.hidden_states,
                                              decoder_attentions=decoder_distractor_outputs.attentions,
                                              cross_attentions=decoder_distractor_outputs.cross_attentions,
                                              encoder_last_hidden_state=encoder_outputs.last_hidden_state,
                                              encoder_hidden_states=encoder_outputs.hidden_states,
                                              encoder_attentions=encoder_outputs.attentions)
                           ]
...
class BlenderbotForConditionalGeneration(BlenderbotPreTrainedModel):
    ...
    def __init__(self, config: BlenderbotConfig):
        ...
        self.distractor_head = nn.Linear(config.d_model, 1)
        self.init_weights()
    ...
    def forward(
        ...
        outputs, outputs_distractor = self.model(input_ids,  attention_mask=attention_mask,
                                                 fact_input_ids=fact_input_ids, #list
                                                 fact_attention_mask=fact_attention_mask, #list
                                                 decoder_input_ids=decoder_input_ids,
                                                 distractor_input_ids=distractor_input_ids,
        ...
        #  outputs[0]: batch, seq len, hid dim
        lm_logits = self.lm_head(outputs[0]) + self.final_logits_bias
        actual_vec = outputs[0] * (input_ids == 2).long().unsqueeze(-1)
        actual_vec = actual_vec.sum(1)  # batch, hid dim
        distractor_vec = outputs_distractor[0] * (distractor_input_ids == 2).long().unsqueeze(-1)
        distractor_vec = distractor_vec.sum(1)  # batch, hid dim
        actual_pred = self.distractor_head(actual_vec)  # batch, 1
        distractor_pred = self.distractor_head(distractor_vec)  # batch, 1
